...
public class MyWebSocketController {
    ...
    private static Map<Integer, Session> sessionHashMap = new HashMap<Integer, Session>();
    ...
    public void onOpen(Session session, @PathParam("uId") Integer uId) {
        ...
        sessionHashMap.put(uId, session);//在建立连接的时候就将用户id和Session绑定
...
/*        //将message转换为SocketMsg对象
        ...
        Session fromSession = sessionHashMap.get(uId);//发送者的Session
        ...
        if (socketMsg.getType() == 1) {
            //单聊，寻找发送者和接受者
            Session toSession = sessionHashMap.get(socketMsg.getToUserId());//接受这的Session
            //将发送内容存入数据库
